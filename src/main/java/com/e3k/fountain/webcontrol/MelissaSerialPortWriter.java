...
public enum MelissaSerialPortWriter implements Runnable {
        ...
        try {
            ...
            for (int i = 0; i < bytes2write.length; i++) {
                ...
                if (++bytesWithinPacketWritten == bytesInPacket) {
                    Static.trySleep(millisDelayBetweenPackets);
    ...
    public synchronized void run() {
        while (true) {
            try {
                ...
                bytes2write = null;
                pel = null;
                SwingUtilities.invokeLater(() -> listener.errorOccurred(Static.formatExceptionMessage(e)));
            }
        }
    }
